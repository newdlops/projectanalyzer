...
1. 전체 Project Learning Journey 로드맵과 현재 vertical slice의 visited progress
2. Map project: 정규화된 rootPath scope 최대 3개
3. 선택된 scope의 lazy Reading Guide
4. Trace one representative request와 Verify in source 행동
5. Explore Code Flows disclosure
6. Browse Structure disclosure
7. Analysis Details disclosure
...
### 7.1 Project Learning Journey, Project Map과 Reading Guide

첫 화면은 전체 학습 로드맵과 현재 다룰 세 행동을 먼저 안내하고, 프로젝트 구조를 빠르게
파악하는 고정 크기 Project Map을 근거 도구로 둔다.

```text
Project Learning Journey
  0 of 3 orientation actions visited · not a readiness score
  Next 1/3 · Map project
    Why -> Learn -> Inspect -> Do -> Explain back -> Exit
  Full onboarding roadmap [collapsed]
    Context -> Architecture -> Critical Flows -> ... -> Continuous Refresh
...
4. 사용자는 기본 화면에서 normalized rootPath 기준 프로젝트 scope와 실행 surface를 최대 3개 카드로 파악할 수 있다.
...
15. scope 선택 전에는 개별 reading path와 Function Index가 Webview에 전달되지 않는다.
